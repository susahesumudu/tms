...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        app_name = 'courses'  # Replace with your app name
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>{{{{ item.name }}}}</td>
          <td>
            <a href=\"{{% url '{app_name}:model_detail' pk=item.pk %}}\">View</a>
            <a href=\"{{% url '{app_name}:model_update' pk=item.pk %}}\">Edit</a>
            <a href=\"{{% url '{app_name}:model_delete' pk=item.pk %}}\">Delete</a>
          </td>
        </tr>
        {% empty %}
    ...
    def generate_model_templates(self, app_name, models):
        ...
        for model in models:
            ...
            self.generate_list_template(base_template_dir, model_name, app_name)
            self.generate_detail_template(base_template_dir, model_name, app_name)
    ...
    # ----------------------
    def generate_list_template(self, base_template_dir, model_name, app_name):
        ...
        if not os.path.exists(template_file_path):
            with open(template_file_path, 'w') as f:
                ...
                f.write(f"<a href=\"{{% url '{app_name}:{model_name}_create' %}}\" class=\"btn btn-sm btn-success mb-3\">Add New</a>\n")
                f.write("<table class=\"table\">\n")
                f.write("  <thead>\n    <tr>\n      <th>ID</th>\n      <th>Name</th>\n      <th>Actions</th>\n    </tr>\n  </thead>\n")
                f.write("  <tbody>\n")
                f.write("    {% for item in items %}\n")
                f.write("    <tr>\n      <td>{{{{ item.id }}}}</td>\n      <td>{{{{ item.name }}}}</td>\n")
                f.write(f"        <a href=\"{{% url '{app_name}:{model_name}_detail' pk=item.pk %}}\">View</a> \n")
                f.write(f"        <a href=\"{{% url '{app_name}:{model_name}_update' pk=item.pk %}}\">Edit</a> \n")
                f.write(f"        <a href=\"{{% url '{app_name}:{model_name}_delete' pk=item.pk %}}\">Delete</a>\n")
                f.write("    </tr>\n    {% empty %}\n    <tr><td colspan=\"3\">No {{ model|lower }} found.</td></tr>\n")
                f.write("    {% endfor %}\n  </tbody>\n</table>\n")
                f.write("{% endblock %}\n")

    def generate_detail_template(self, base_template_dir, model_name, app_name):
